...
class HVideoContainer extends React.Component{
    ...
    componentDidUpdate(prevProps , prevState) {//每次render结束后会触发
        if(this.userIDArryNumber !==  this.state.userIDArry.length){
            ...
            this.setState({childrenSplitScreenStyle:SplitScreen.update(this.state.userIDArry.length,this.state.childrenSplitScreenStyle)});
            if(TkConstant.joinRoomInfo.areaExchange &&  TkConstant.hasRoomtype.oneToOne ){
                let user=ServiceRoom.getTkRoom().getMySelf();
                if(this.state.userIDArry.length >= 1){
                ...
                }else if(this.state.userIDArry.length ===0){
                    eventObjectDefine.CoreController.dispatchEvent({
                        type:'areaExchangeClose',
                        message: {
                            hasExchange: false,
        ...
        /*为卡通新模板全部恢复按钮特此判断处理*/
        if(Object.keys(this.state.otherVideoStyle).length > 0){
            let newArray = Object.values(this.state.otherVideoStyle).map((item,index)=>{
                if(item.isDrag || this.state.userIDArry.length > 0){
                    return 'contain'
                }else{
                    return false
                }
            });
            if(newArray.filter(i=>i==='contain').length == 0 && this.state.userIDArry.length == 0){
                if(this.flag === true){
    ...
    handleReceiveMsglistVideoSplitScreen(recvEventData){
        ...
        this.setState({userIDArry: userIDArry});
        TkGlobal.isSplitScreen = this.state.userIDArry.length > 0 ; //根据数组长度改变是否是分屏状态
    };
    ...
    handlerRoomParticipantLeave(handleData) {//用户离开房间时，删除该视频框的样式
        ...
        if(userIDArryCopy.indexOf(user.id)>=0){
        	...
        	let users = ServiceRoom.getTkRoom().getUsers();//用于判断是否有老师或助教
        	for (let [key,item] of Object.entries(users)) {
        	    if (item.role === TkConstant.role.roleChairman || item.role === TkConstant.role.roleTeachingAssistant) {
        	        hasTeacher = true;
        	        break;
                }
            }
            //如果有老师或助教则由老师或助教发此信令，没有则所有人发此信令
            let toID = '__none';
            SplitScreen.sendSignalling(userIDArryCopy, !hasTeacher, toID);
        }
    };
    ...
    handlerRoomUserpropertyChanged (recvEventData) {//用户属性改变的时候
        ...
        for (let [key,value] of Object.entries(changeUserproperty)) {
            if (key === "publishstate" && value === TkConstant.PUBLISHSTATE.PUBLISH_STATE_NONE && (TkConstant.hasRole.roleChairman || TkConstant.hasRole.roleTeachingAssistant)) { //如果是下台状态并且是老师或者助教，才可以发删除该视频框的样式信令
                //其他人下台将他的视频样式数据删除
                ...
                if(userIDArryCopy.indexOf(user.id) >= 0 ) {
                    ...
                    SplitScreen.sendSignalling(userIDArryCopy);
                }
            }
        }
    };
    ...
    changeOtherVideoStyle(handleData) {
        ...
        if(this.state.userIDArry.indexOf(extensionId) === -1  &&  this.state.userIDArry.length >= (TkGlobal.isVideoInTop? this.maxVideoNumber:this.maxVideoNumber-1)){
        	return
        }
        let otherVideoStyleCopy = {};
        if (this.state.streams) {
    ...
    addStream(stream) {
        ...
        if (!this.findStream(stream)) {
                if(this.getUser(stream.extensionId)  && this.getUser(stream.extensionId).role === TkConstant.role.roleChairman){
                    if(TkConstant.template === 'template_classic'){
    ...
    upDateChange(message){
        ...
        if( _index!== -1 && this.state.streams[_index].mediaFlagObj.videoFlag === false && this.state.streams[_index].mediaFlagObj.audioFlag === false ){
            if(!ServiceRoom.getTkRoom().getUser(message.userId) || (ServiceRoom.getTkRoom().getUser(message.userId) &&(ServiceRoom.getTkRoom().getUser(message.userId).publishstate  !== TkConstant.PUBLISHSTATE.PUBLISH_STATE_MUTEALL))){
                this.setState({
                    streams:this.state.streams.filter((item,index)=>index!==_index)
                });
	...
	changeMainContentVesselSmartSize(data){
        SplitScreen.update(this.state.userIDArry.length,this.state.childrenSplitScreenStyle);
    ...
    videoOnDoubleClick(stream, extensionId , event) {
        if(TkGlobal.isVideoInFullscreen){
                //todu    产品确认开启画中画模式后，一对多双击右下角视频不全屏
        }else{
            if(CoreController.handler.getAppPermissions('isCanDragVideo') && TkGlobal.classBegin) {
                if( (this.state.userIDArry.indexOf(extensionId) === -1  &&  this.state.userIDArry.length >= (TkGlobal.isVideoInTop?this.maxVideoNumber:this.maxVideoNumber-1)) || !(TkConstant.hasRole.roleChairman ||  TkConstant.hasRole.roleTeachingAssistant) ){
                    return;
                };
                if(stream){
                    let userIDArryCopy =  this.state.userIDArry.slice(0,this.state.userIDArry.length) ;
                    if(TkGlobal.isSplitScreen) { //处于分屏模式
                        //if(!TkConstant.hasRoomtype.oneToOne ){
    ...
    _getInitVideoSize() {
        ...
        let initVideoWidth = ((lcVideoContainer.clientWidth / defalutFontSize) / TkGlobal.videoNum - 0.15);
    ...
    _initVideoSizeByUserid(extensionId) {
        const defalutFontSize = TkGlobal.windowInnerWidth / TkConstant.STANDARDSIZE;
        let lcVideoContainer = document.getElementById('other_video_container');
        let videoWidth = ((lcVideoContainer.clientWidth / defalutFontSize) / TkGlobal.videoNum - 0.15);
        let videoHeight = videoWidth / TkGlobal.videoScale;
    ...
    _loadHVideoComponentArray( streams ){
        ...
        for(let stream of streams){
            if(stream) {
                ...
                let videoContainerStyle = SplitScreen.load(extensionId,this.state.userIDArry,this.state.childrenSplitScreenStyle);
                let isSplitScreenFromStream = false ;
                if( this.state.userIDArry.indexOf(extensionId) >=0 ){
                    isSplitScreenFromStream = true ;
                }
                ...
                hVideoComponentArray.push(
                    <CommonVideoSmart sendSignallingOfVideoSize={this.sendSignallingOfVideoSize.bind(this)}
                                      changeVideoSize={this.changeVideoSize.bind(this)} isSplitScreenFromStream={isSplitScreenFromStream}
                                      ...
                                      id={extensionId}  key={stream.extensionId} stream={stream} videoDumbClassName={"hvideo"}
                                      videoOnDoubleClick={this.videoOnDoubleClick.bind(this , stream ,extensionId)} zhikeTeachingAssistantMute={this.state.zhikeTeachingAssistantMute} 
                                      pictureInPictureClassname={ TkConstant.role.roleChairman===userRole ? this.state.pictureInPictureClassnameFromTeacherStream : ""} />
                );
            }
        }
        return{
