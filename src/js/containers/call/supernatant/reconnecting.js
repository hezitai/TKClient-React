...
class ReconnectingSmart extends React.Component{
    ...
    componentDidMount() { //在完成首次渲染之前调用，此时仍可以修改组件的state
        let that = this ;
        eventObjectDefine.CoreController.addEventListener( TkConstant.EVENTTYPE.RoomEvent.roomDisconnected , that.handlerDisconnected.bind(that)  , that.listernerBackupid );
    ...
    render(){
        ...
        return (
            <LoadSupernatantPromptDumb  show={reconnecting} content ={TkGlobal.language.languageData.loadSupernatantPrompt.reconnecting} {...that.props}/>
        )
    }
};
