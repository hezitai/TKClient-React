...
class SupernatantDynamicPptVideoSmart extends React.Component{
    ...
    handlerPlayDynamicPPTMediaStream(recvEventData){
        const that = this ;
        let {show} = recvEventData.message ;
    ...
    render(){
        ...
        return (
            <section className="dynamicPptVideoPlaying-container add-position-absolute-top0-left0" id="dynamicPptVideoPlaying_container"   style={{display:dynamicPptVideoPlaying?'block':'none',width:'100%' , height:'100%' , backgroundColor:'rgba(0,0,0,0.5)' , zIndex:205 , }} />
        )
    }
};
