...
class sharingScreen extends React.Component {
	...
	handleMovieShareClick() {
		...
		if (TkGlobal.isClient) {
			...
			ServiceRoom.getNativeInterface().getOpenFileName(
				{
					caption: "Media File Selector",
					filter: {
						"Media File":
							"*.mp4;*.3gp;*.avi;*.flv;*.mkv;*.mov;*.mpg;*.rmvb;*.vob;*.wmv"
					}
				},
				arg => {
					if (arg.action === "Complete") {
						// Complete Cancelled Activated
						ServiceTools.stopAllMediaShare();
						ServiceRoom.getNativeInterface().startShareMediaFile(
							arg.filename,
							undefined,
							undefined,
							{
...
sharingScreen = WrapWithUpdate(sharingScreen);
