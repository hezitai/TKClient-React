...
class Loading extends React.Component {
    ...
    componentWillUnmount() { //组件被移除之前被调用，可以用于做一些清理工作，在componentDidMount方法中添加的所有任务都需要在该方法中撤销，比如创建的定时器或添加的事件监听器//'#121a2c'
        clearTimeout(this.timer);
        this.timer = null ;
    };
    render() {
        return (
            <div className="tk-download-loading" style={{display:this.state.show?'block':'none' , width:'100%' , height:'100%' , position: 'absolute' ,  left:0 , top:0 , backgroundColor:' #F1F4F7'}}>
