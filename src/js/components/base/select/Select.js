...
class SelectDumb extends React.Component {
    ...
    render() {
        const { id , className ,optionClassName , selectOptions = []  ,  disabled ,isMobile = false  , currentValue , maxH } = this.props  ;
        let currentLabel =  undefined ;
        for(var selectOption of selectOptions ){
            if( selectOption.value === currentValue ){
        ...
        return (
            <div id={id} ref={"tkSelectContainer"} className={"tk-select-container clear-float " + this.state.listDirection +(disabled?' disabled ':'  ') + (className || '') + ( isMobile ? 'app-mobile ' : ' ' )  + (disabled ? ' hideExtendList':(this.state.extendShow?' showExtendList':' hideExtendList') ) }  onMouseLeave={isMobile?undefined:this.selectContainerOnMouseLeave.bind(this)} >
                <div className={"current-select-text "+(disabled?'disabled ':' ')} disabled={disabled}  onClick={disabled?undefined:this.currentTextOnClick.bind(this)} >{currentLabel}</div>
                <span className={"arrow-container " +(disabled?'disabled ':' ') } onClick={disabled?undefined:this.currentTextOnClick.bind(this)}>
                    <em className={"arrow "}></em>
                </span>
                <div className={"select-extend-list-container-box " + this.state.listDirection + (disabled?' disabled ':'  ')} style={{display:disabled?'none':(this.state.extendShow?'block':'none') , maxHeight:maxH?"1.45rem":"calc("+(this.maxOptionNumber)+" * 0.5rem)" }}>
                    <ol className={"select-extend-list-container " + this.state.listDirection + (disabled?' disabled ':'  ')}  style={{display:disabled?'none':(this.state.extendShow?'block':'none') , maxHeight:maxH?"1.25rem":"calc("+(this.maxOptionNumber)+" * 0.5rem)" }} >
                        {selectOptions.map( (item , index) => {
                            return (  <li key={index} className={"select-option "  + (optionClassName || ' ') + (currentValue === item.value?' selected':' ' )}  tkcustomdatalabel={item.label} onClick={disabled?undefined:this.optionOnClick.bind(this , item.value , index)}  >{item.label}</li> ) ;
                        })}
                    </ol>
                </div>
