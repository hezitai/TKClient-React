...
const tkUtils = {
    ...
    /*去掉px，转成数字*/
    replacePxToNumber: (remStr) => {
        if (remStr && typeof remStr === 'string') {
            remStr = Number(remStr.replace(/px/g, ""));
        }
        return remStr;
    },
    /*去掉calc()*/
    ...
    getSpecifyRoleList: function(specifyRoleIdentification, allUser) {
        if (specifyRoleIdentification === undefined || allUser === undefined) {
            return {};
        }
        let rolelist = {};
        for (let user of Object.values(allUser)) {
            if (!rolelist[user.role]) { rolelist[user.role] = {} };
            rolelist[user.role][user.id] = user;
        }
        let specifyRole = rolelist[specifyRoleIdentification] || {};
        return specifyRole;
    },
    createBurglarChain(liveurlpath, time) { //创建时间戳防盗链
        /*
        时间戳防盗链：域名
     key： ***********
     时间格式：10进制
     加密串: md5(secretkey+uri+wstime) 
     有效期：可配，自定义
     防盗链URL格式：
     比如：http://域名/live/路径?wsSecret=加密串&wsTime=time
              其中wsTime=当前时间戳+过期时间； wsSecret =md5(secretkey+uri+wstime)，  md5采用32位小写
        URI是指： /live/路径
         */
        //防盗链参数
        let talkcloudkey = "e32abcd53d6839164952931f26dc66ec"; //企业key，网宿分配的
        // let salt=(Date.parse(new Date()))/1e3;
        let salt = time;
        let wsTime = salt + 300;
        let wsSecret = Md5(talkcloudkey + liveurlpath + wsTime);
        return { wsSecret: wsSecret, wsTime: wsTime };
    },
};
